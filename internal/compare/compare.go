...
func (c *Comparator) CompareReports(report1Path, report2Path string) (ComparisonResult, error) {
	...
	result.OverallSuccessRateChange = float64(report2.WorkingConfigs)/float64(max(report2.ValidConfigs, 1))*100 -
		float64(report1.WorkingConfigs)/float64(max(report1.ValidConfigs, 1))*100
