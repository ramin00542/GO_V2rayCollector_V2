...
func (u *Updater) updateAll(ctx context.Context) error {
	...
	var subscriptions []repository.Source
	for _, source := range sources {
		if source.Enabled && source.Kind == "subscription" {
...
func (u *Updater) updateSubscription(ctx context.Context, sub repository.Source) error {
	u.mu.Lock()
	defer u.mu.Unlock()
